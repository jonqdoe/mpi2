...
void swap_particles() {
  ...
  for ( send_proc=0 ; send_proc < nprocs ; send_proc++ ) {
    ...
    if ( send_proc == myrank ) {
      ...
      for ( i=0 ; i<Sbound_ct ; i++ )
        local_flag[ send_S_inds[i] ] = 0 ;
  ...
  for ( send_proc=0 ; send_proc < nprocs ; send_proc++ ) {
    ...
    if ( send_proc == myrank ) {
      ...
      for ( i=0 ; i<Nbound_ct ; i++ )
        local_flag[ send_N_inds[i] ] = 0 ;
